...
from PySide6.QtWidgets import (
    ...
    QGridLayout, QFrame, QGroupBox, QTabWidget, QMessageBox
)
...
class OverviewTab(CMCTabWidget):
    ...
    def _build_gui(self) -> None:
        ...
        info_label.setAlignment(Qt.AlignRight | Qt.AlignTop)
        info_font = QFont()
        info_font.setPointSize(10)
        info_label.setFont(info_font)
        info_label.setStyleSheet(f"color: {COLOR_DEFAULT};")
        ...
        if manager:
            if manager.name == "Mod Organizer":
                ...
                label_mod_manager_icon.setCursor(QCursor(Qt.PointingHandCursor))
                ...
                if self.cmc.pc.os == "Windows 11 24H2" and manager.version <= Version("2.5.2"):
                    ...
                    top_layout.addWidget(label_os_icon, 3, 1, Qt.AlignTop | Qt.AlignLeft)
            ...
            top_layout.addWidget(label_mod_manager_icon, 0, 1, Qt.AlignLeft)
        
        # Mod manager text
        label_mod_manager = QLabel(
            f"{manager.name} v{manager.version} [Profile: {manager.selected_profile or 'Unknown'}]"
            if manager
            else "Not Found"
        )
        label_mod_manager.setFont(info_font)
        label_mod_manager.setStyleSheet(f"color: {COLOR_NEUTRAL_2 if manager else COLOR_BAD};")
        top_layout.addWidget(label_mod_manager, 0, 2, Qt.AlignLeft)
        if not manager:
            label_mod_manager.setToolTip(TOOLTIP_NO_MOD_MANAGER)
        
        # Game path
        label_path = QLabel()
        label_path.setFont(info_font)
        label_path.setStyleSheet(f"color: {COLOR_NEUTRAL_2};")
        label_path.setCursor(QCursor(Qt.PointingHandCursor))
        self.cmc.game_path_sv.changed.connect(label_path.setText)
        label_path.setText(self.cmc.game_path_sv.get())
        label_path.mousePressEvent = lambda e: os.startfile(self.cmc.game.game_path)
        label_path.setToolTip(TOOLTIP_GAME_PATH)
        top_layout.addWidget(label_path, 1, 2, Qt.AlignLeft)
        
        # Version
        label_version = QLabel()
        label_version.setFont(info_font)
        label_version.setStyleSheet(f"color: {COLOR_GOOD};")
        self.cmc.install_type_sv.changed.connect(label_version.setText)
        label_version.setText(self.cmc.install_type_sv.get())
        top_layout.addWidget(label_version, 2, 2, Qt.AlignLeft)
        ...
        label_spec1.setStyleSheet(f"color: {COLOR_NEUTRAL_2};")
        ...
        label_spec2.setStyleSheet(f"color: {COLOR_NEUTRAL_2};")
        ...
        button_refresh.setToolTip(TOOLTIP_REFRESH)
        top_layout.addWidget(button_refresh, 0, 3, 2, 1, Qt.AlignRight)
    ...
    def build_gui_binaries(self) -> None:
        ...
        label_file_names.setAlignment(Qt.AlignRight)
        font = QFont()
        font.setPointSize(10)
        label_file_names.setFont(font)
        label_file_names.setStyleSheet(f"color: {COLOR_DEFAULT};")
        binaries_layout.addWidget(label_file_names, 0, 0, rows, 1)
        
        # Address Library label
        label_al_title = QLabel("Address Library:")
        label_al_title.setAlignment(Qt.AlignRight)
        label_al_title.setFont(font)
        label_al_title.setStyleSheet(f"color: {COLOR_DEFAULT};")
        binaries_layout.addWidget(label_al_title, rows, 0)
        
        label_address_library = QLabel(
            "Not Found" if not self.cmc.game.address_library else 
            "Next-Gen" if self.cmc.game.is_fong() else "Old-Gen"
        )
        label_address_library.setFont(font)
        color = COLOR_GOOD if self.cmc.game.address_library else COLOR_BAD
        ...
        if not self.cmc.game.address_library:
            label_address_library.setToolTip(TOOLTIP_ADDRESS_LIBRARY_MISSING)
        ...
        for i, file_name in enumerate(self.cmc.game.file_info.keys()):
                    ...
                    if self.cmc.game.is_fodg():
                        ...
                        self.cmc.overview_problems.append(
                            ProblemInfo(
                                ...
                                file_path,
                                file_path.relative_to(self.cmc.game.game_path),
                                None,
                                f"{file_name} is Old-Gen and should be Next-Gen or downgraded.",
                                SolutionType.Downgrade,
                            )
                        )
                case InstallType.NG:
                    if self.cmc.game.is_fong():
                        ...
                        self.cmc.overview_problems.append(
                            ProblemInfo(
                                ...
                                file_path.relative_to(self.cmc.game.game_path),
                                None,
                                f"{file_name} is Next-Gen and should be Old-Gen or downgraded.",
                                SolutionType.Delete,
                            )
                        )
                case _:
            ...
            label_version = QLabel(
                ver_to_str(self.cmc.game.file_info[file_name]["Version"])
                if self.cmc.game.file_info[file_name]["Version"]
                else "Not Found"
            )
            ...
            if color == COLOR_BAD:
                ...
                binaries_layout.addWidget(icon_label, i, 2, Qt.AlignLeft)
        ...
        for archive, version in self.cmc.game.archives.items():
            ...
            if archive_type == "Texture":
                ...
                if version == ArchiveVersion.OG:
                    colors_texture.append(COLOR_GOOD if self.cmc.game.is_fodg() else COLOR_BAD)
                else:
                    colors_texture.append(COLOR_GOOD if self.cmc.game.is_fong() else COLOR_BAD)
            else:
                lines_main.append(f"{base_name}:")
                if version == ArchiveVersion.OG:
                    colors_main.append(COLOR_GOOD if self.cmc.game.is_fodg() else COLOR_BAD)
                else:
                    colors_main.append(COLOR_GOOD if self.cmc.game.is_fong() else COLOR_BAD)
        
        # Create labels for archives
        if lines_texture:
            label_textures = QLabel("\n".join(lines_texture))
            label_textures.setAlignment(Qt.AlignRight)
            font = QFont()
            font.setPointSize(10)
            ...
            if all(c == colors_texture[0] for c in colors_texture):
                ...
                label_textures.setStyleSheet(f"color: {COLOR_DEFAULT};")
            archives_layout.addWidget(label_textures, 0, 0, len(lines_texture), 1)
            
            # Version labels
            for i, (line, color) in enumerate(zip(lines_texture, colors_texture)):
                version_label = QLabel("NG" if self.cmc.game.archives[f"{line[:-1]} - Textures.ba2"] == ArchiveVersion.NG else "OG")
    ...
    def build_gui_modules(self) -> None:
        ...
        label_module_types.setAlignment(Qt.AlignRight)
        label_module_types.setFont(font)
        label_module_types.setStyleSheet(f"color: {COLOR_DEFAULT};")
        label_module_types.setToolTip(TOOLTIP_MODULE_TYPES)
        modules_layout.addWidget(label_module_types, 0, 0, 3, 1)
        
        # Module counts
        modules_layout.addWidget(self._create_label(str(self.cmc.game.modules_full), font, COLOR_NEUTRAL_2), 0, 1)
        modules_layout.addWidget(self._create_label(str(self.cmc.game.modules_light), font, COLOR_NEUTRAL_2), 1, 1)
        modules_layout.addWidget(self._create_label(str(self.cmc.game.modules_total), font, COLOR_NEUTRAL_2), 2, 1)
        
        # Unreadable
        color_unreadable = COLOR_BAD if self.cmc.game.modules_unreadable else COLOR_NEUTRAL_1
        label_unreadable = QLabel("Unreadable:")
        label_unreadable.setAlignment(Qt.AlignRight)
        label_unreadable.setFont(font)
        label_unreadable.setStyleSheet(f"color: {color_unreadable};")
        label_unreadable.setToolTip(TOOLTIP_UNREADABLE)
        ...
        label_hedr_100.setAlignment(Qt.AlignRight)
        label_hedr_100.setFont(font)
        label_hedr_100.setStyleSheet(f"color: {COLOR_DEFAULT};")
        label_hedr_100.setToolTip(TOOLTIP_HEDR_100)
        modules_layout.addWidget(label_hedr_100, 5, 0)
        modules_layout.addWidget(self._create_label(str(self.cmc.game.modules_hedr_100), font, COLOR_NEUTRAL_2), 5, 1)
        
        label_hedr_95 = QLabel("HEDR v0.95:")
        label_hedr_95.setAlignment(Qt.AlignRight)
        label_hedr_95.setFont(font)
        label_hedr_95.setStyleSheet(f"color: {COLOR_DEFAULT};")
        label_hedr_95.setToolTip(TOOLTIP_HEDR_95)
        modules_layout.addWidget(label_hedr_95, 6, 0)
        modules_layout.addWidget(self._create_label(str(self.cmc.game.modules_hedr_95), font, COLOR_NEUTRAL_2), 6, 1)
        
        # HEDR unknown
        color_hedr_unknown = COLOR_BAD if self.cmc.game.modules_hedr_unknown else COLOR_NEUTRAL_1
        label_hedr_unknown = QLabel("HEDR v????:")
        label_hedr_unknown.setAlignment(Qt.AlignRight)
        label_hedr_unknown.setFont(font)
        label_hedr_unknown.setStyleSheet(f"color: {color_hedr_unknown};")
        label_hedr_unknown.setToolTip(TOOLTIP_HEDR_UNKNOWN)
    ...
    def get_info_modules(self, *, refresh: bool = False) -> None:
        ...
        for module_path in self.cmc.game.modules_enabled:
            ...
            if ModuleFlag.ESL in flags:
                self.cmc.game.modules_light += 1
            else:
                self.cmc.game.modules_full += 1
            self.cmc.game.modules_total += 1

            hedr_version = struct.unpack("<f", head[30:34])[0]
            if hedr_version == 1.0:
                self.cmc.game.modules_hedr_100 += 1
            elif hedr_version == 0.95:
                self.cmc.game.modules_hedr_95 += 1
            else:
                self.cmc.game.modules_hedr_unknown[module_path.stem] = f"{hedr_version:.4f}".removesuffix("00")
