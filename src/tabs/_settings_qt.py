...
from PySide6.QtWidgets import QButtonGroup, QGroupBox, QRadioButton, QTabWidget, QVBoxLayout
	...
	def _build_gui(self) -> None:
		...
		for name, (tooltip, column, row, var, action, options) in options_radios.items():
			...
			for text, value in options:
				...
				radio.toggled.connect(lambda checked, s=action, v=value: self.on_radio_change(s, v) if checked else None) # pyright: ignore[reportUnknownLambdaType, reportUnknownArgumentType]

				button_group.addButton(radio)
				group_layout.addWidget(radio)

			# Add to main layout
			self.main_layout.addWidget(group_box, row, column, Qt.AlignmentFlag.AlignTop)
