...
class ModalDialogBase(QDialog):
	"""Base class for modal dialogs."""

	def __init__(self, parent: QWidget, cmc: "CMCheckerInterface", window_title: str, width: int, height: int) -> None:
	...
	def setup_window(self) -> None:
		...
		if parent_widget:
			parent_rect: QRect = parent_widget.geometry()
			x = parent_rect.center().x() - self._width // 2
			y = parent_rect.center().y() - self._height // 2
			self.move(x, y)
...
class AboutDialog(ModalDialogBase):
	...
	def __init__(self, parent: QWidget, cmc: "CMCheckerInterface", width: int, height: int, title: str, text: str) -> None:
	...
	def build_gui(self) -> None:
		...
		self.setWindowFlag(Qt.WindowType.WindowCloseButtonHint, True)  # noqa: FBT003
...
class TreeDialog(ModalDialogBase):
	...
	def __init__(
		...
		items: list[tuple[Any, str | Path]] | None = None,
	) -> None:
		self.info_text = text
	...
	def populate_tree(self) -> None:
		...
		sorted_items = sorted(self.items, key=operator.itemgetter(0), reverse=True)
