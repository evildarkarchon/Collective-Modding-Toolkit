...
class BaseWorker(QObject):
	...
	def run(self) -> None:
		...
		try:
			...
			self.signals.error_occurred.emit(str(e), traceback.format_exc())
		finally:
			with QMutexLocker(self._mutex):
				self._is_running = False
...
class ThreadManager(QObject):
	...
	def start_worker(
		...
		with QMutexLocker(self._mutex):
			...
			logger.debug("Started thread: %s", thread_name)
			return thread

	def stop_thread(self, thread_name: str, wait: bool = True, timeout: int = 5000) -> None:
		...
		if wait and thread.isRunning() and not thread.wait(timeout):
			logger.warning("Thread %s did not stop gracefully", thread_name)
			thread.terminate()
			thread.wait()

	def stop_all(self, wait: bool = True, timeout: int = 5000) -> None:
	...
	def get_active_threads(self) -> list[str]:
		...
		with QMutexLocker(self._mutex):
			return [name for name, (thread, _) in self._threads.items() if thread.isRunning()]
	...
	def _cleanup_thread(self, thread_name: str) -> None:
		...
		with QMutexLocker(self._mutex):
			if thread_name in self._threads:
				...
				logger.debug("Cleaned up thread: %s", thread_name)
...
def run_in_thread(
	thread_manager: ThreadManager, thread_name: str | None = None, priority: QThread.Priority = QThread.Priority.NormalPriority,
) -> Callable[[Callable[..., object]], Callable[P, BaseWorker]]:
	...
	def decorator(func: Callable[..., object]) -> Callable[P, BaseWorker]:
		@wraps(func)
		def wrapper(*args: P.args, **kwargs: P.kwargs) -> BaseWorker:  # pyright: ignore[reportUnusedFunction]
			class FunctionWorker(BaseWorker):
				def execute(self) -> object:
					return func(self.update_progress, self.update_status, *args, **kwargs)
