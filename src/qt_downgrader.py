...
from enums import LogType, Tab, InstallType
from globals import *
...
class DownloadWorker(WorkerBase):
    ...
    def __init__(self, url: str, infile: Path, outfile: Path):
        super().__init__()
        self.url = url
        self.infile = infile
...
class QtDowngrader(ModalDialogBase):
    ...
    def show_about(self) -> None:
        ...
        dialog = AboutDialog(self, self.cmc, 500, 300, 
                           ABOUT_DOWNGRADING_TITLE, ABOUT_DOWNGRADING)
        dialog.exec()
    ...
    def patch_file(self, file_path: Path, desired_version: InstallType) -> None:
        ...
        try:
            ...
            self.logger.log_message(LogType.Bad, f"Failed patching {file_path.name}: {str(e)}")
    ...
    def process_download_queue(self) -> None:
        ...
        if not self.download_queue:
            ...
            self.get_info()
            parent_layout = self.layout().itemAt(0).layout()  # Get top layout
            game_layout = parent_layout.itemAt(0).widget().layout()  # Game group layout
            ck_layout = parent_layout.itemAt(1).widget().layout()  # CK group layout
            self.populate_version_labels(game_layout, ck_layout)
            self.button_patch.setEnabled(True)
        ...
        if is_file(file_path):
            ...
            self.download_thread = DownloadWorker(url, infile, outfile)
    ...
    @Slot(str, Path, Path)
    def on_download_complete(self, url: str, infile: Path, outfile: Path):
